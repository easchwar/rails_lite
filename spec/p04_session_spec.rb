require 'webrick'
require 'aux/session'
require 'base/controller_base'

describe Controller::Session do
  let(:req) { WEBrick::HTTPRequest.new(Logger: nil) }
  let(:res) { WEBrick::HTTPResponse.new(HTTPVersion: '1.0') }
  let(:cook) { WEBrick::Cookie.new('_rails_lite_session', { xyz: 'abc' }.to_json) }

  it "deserializes json cookie if one exists" do
    req.cookies << cook
    session = Controller::Session.new(req)
  ...
  describe "#store_session" do
    context "without cookies in request" do
      ...
      it "adds new cookie with '_rails_lite_session' name to response" do
        cookie = res.cookies.find { |c| c.name == '_rails_lite_session' }
    ...
    context "with cookies in request" do
      before(:each) do
        cook = WEBrick::Cookie.new('_rails_lite_session', { pho: "soup" }.to_json)
        req.cookies << cook
      end
...
describe Controller::Base do
  before(:all) do
    class CatsController < Controller::Base
    end
  end
  ...
  describe "#session" do
    it "returns a session instance" do
      expect(cats_controller.session).to be_a(Controller::Session)
    end
